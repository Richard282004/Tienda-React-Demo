...
export function PriceCalculator() {
  ...
  const [values, setValues] = useState({ costs: '', shippingCost: '0', shippingCharged: '0', feePercent: '', fixedFee: '0', inputVatCredit: '0', reservePercent: '0', hours: '1', hourlyPay: '', extraProfit: '0', currentPrice: '' });
  ...
  const input: PricingInput = { costs: Number(values.costs), shippingCost: Number(values.shippingCost), shippingCharged: Number(values.shippingCharged), feePercent: Number(values.feePercent), fixedFee: Number(values.fixedFee), feeIncludesVat, vatPercent: tax === 'iva' ? 19 : 0, inputVatCredit: tax === 'iva' ? Number(values.inputVatCredit) : 0, reservePercent: Number(values.reservePercent), hours: Number(values.hours), hourlyPay: Number(values.hourlyPay), extraProfit: Number(values.extraProfit) };
  const requiredFields = ['costs', 'feePercent', 'hours', 'hourlyPay'] as const;
  const ready = tax !== '' && requiredFields.every((key) => values[key].trim() !== '');
  ...
  return <section className="price-calculator">
    ...
    <div className="pricing-layout"><div className="pricing-form">
      <section><h3>Tu producto y tu trabajo</h3><div className="pricing-fields">
        {field('costs', 'Materiales, empaque y otros costos ($)', 'Total por unidad, incluido el IVA que pagaste. Incluye una parte de tus gastos fijos.')}
        {field('hours', 'Horas de trabajo por unidad', 'Por ejemplo, 0,5 para media hora.')}
        {field('hourlyPay', 'Cuánto quieres recibir por hora ($)')}
        {field('extraProfit', 'Ganancia adicional por unidad ($)', 'Además del pago por tu tiempo.')}
      </div></section>
      ...
      {!ready && <p>Completa costos, pago por hora, comisión y tratamiento de IVA para ver el resultado.</p>}
      ...
      {result && suggested !== null && <><span>Precio sugerido al cliente</span><strong className="pricing-total">{money(suggested)}</strong><p>Precio final del producto{tax === 'iva' ? ', IVA incluido' : ''}. Redondeado hacia arriba a $100.</p>
        <dl><div><dt>Total cobrado, con envío</dt><dd>{money(result.collected)}</dd></div><div><dt>Comisión, incluido su IVA</dt><dd>−{money(result.fees)}</dd></div><div><dt>Costos y envío que pagas</dt><dd>−{money(input.costs + input.shippingCost)}</dd></div><div><dt>Reserva para IVA</dt><dd>−{money(result.vatProvision)}</dd></div><div><dt>Reserva para renta elegida</dt><dd>−{money(result.reserve)}</dd></div><div className="pricing-pocket"><dt>Disponible estimado para ti</dt><dd>{money(result.pocket)}</dd></div><div><dt>De ese saldo, pago por tu tiempo</dt><dd>{money(result.timePay)}</dd></div><div><dt>Resto después de valorar tu tiempo</dt><dd>{money(result.afterTime)}</dd></div></dl>
        <p>El disponible incluye el pago por tu trabajo. No equivale a una utilidad neta tributaria definitiva.</p>
