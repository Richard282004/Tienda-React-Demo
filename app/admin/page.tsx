    ...
    <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as string)} className="admin-tabs" orientation="vertical">
      ...
      <TabsContent value="content">
        <form className="content-editor" onSubmit={saveContent}><div className="admin-section-heading"><div><h2>Textos y contacto</h2><p>Cambia el contenido principal sin tocar código.</p></div><Button disabled={busy} type="submit"><Save size={17} /> Guardar cambios</Button></div>
          ...
          <section><h3>Contacto y envíos</h3><div className="form-grid"><label>Teléfono<Input type="tel" inputMode="tel" value={content.phone} onChange={(event) => setContent({ ...content, phone: event.target.value })} /></label><label>Correo<Input type="email" value={content.email} onChange={(event) => setContent({ ...content, email: event.target.value })} /></label><label>WhatsApp (con código de país, sin espacios)<Input type="tel" inputMode="tel" value={content.whatsapp ?? ''} placeholder="56912345678" onChange={(event) => setContent({ ...content, whatsapp: event.target.value })} /></label><label className="full">Correo para avisos de nuevas ventas (admin)<Input type="email" value={content.orderNotifyEmail ?? ''} placeholder="tu@correo.com" onChange={(event) => setContent({ ...content, orderNotifyEmail: event.target.value })} /></label><label className="full">Mensaje superior<Input value={content.shippingMessage} onChange={(event) => setContent({ ...content, shippingMessage: event.target.value })} /></label><label className="full">Llamado a la acción del pie de página<Input value={content.footerCta} onChange={(event) => setContent({ ...content, footerCta: event.target.value })} /></label></div>
            <p className="admin-section-note">Cada vez que se confirme el pago de un pedido nuevo, se enviará un aviso a este correo. Déjalo vacío para no recibir avisos.</p>
          </section>
