...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    <header className="admin-header"><div><p className="admin-kicker">{content.brandName} · Panel privado</p><h1>Administración de la tienda</h1></div><div><a href="/">Ver tienda ↗</a><PushAdmin /><Button variant="outline" onClick={logout}><LogOut size={16} /> Salir</Button></div></header>
    ...
    <Tabs value={activeTab} onValueChange={(value) => selectTab(value as string)} className="admin-tabs" orientation="vertical">
      ...
      <TabsContent value="content">
        <form className="content-editor" onSubmit={saveContent}><div className="admin-section-heading"><div><h2>Textos y contacto</h2><p>Cambia el contenido principal sin tocar código.</p></div><Button disabled={busy} type="submit"><Save size={17} /> Guardar cambios</Button></div>
          ...
          <details className="admin-collapse"><summary>Categorías y moneda</summary><p className="admin-section-note">Agrega, ordena o quita las categorías de producto que quieras vender, sin tocar código.</p>
            <div className="category-manager">
              ...
              <div className="category-add">
                <Input value={newCategoryName} onChange={(event) => setNewCategoryName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); addCategoryItem(); } }} placeholder="Nueva categoría (ej: Tazas)" />
                <button type="button" className="category-add-button" onClick={addCategoryItem}><Tag size={14} /> Agregar</button>
              </div>
            </div>
          ...
          <details className="admin-collapse"><summary>Cotización de envío en tiempo real</summary><p className="admin-section-note">Cotiza el despacho con couriers reales (vía Envia) según la dirección del cliente, en vez de solo el precio fijo por región. La API key se configura como secreto en Cloudflare (ENVIA_API_TOKEN).</p>
            <div className="form-grid">
              <label className="shipping-rate-address-toggle full"><input type="checkbox" checked={content.courierQuoteEnabled ?? false} onChange={(event) => setContent({ ...content, courierQuoteEnabled: event.target.checked })} /> Activar cotización real en el checkout</label>
              <label>Peso promedio por unidad (gramos)<Input type="number" inputMode="numeric" min={1} value={content.shippingDefaultItemWeightGrams ?? 150} onChange={(event) => setContent({ ...content, shippingDefaultItemWeightGrams: Number(event.target.value) || 150 })} /><small className="password-hint">Se usa para estimar el peso del paquete (unidades del carrito × este peso). Los llaveros y peluches chicos rondan 100-250 g.</small></label>
              <label>Calle<Input value={content.shippingOriginStreet ?? ''} placeholder="Araucanía" onChange={(event) => setContent({ ...content, shippingOriginStreet: event.target.value })} /></label>
              <label>Número<Input value={content.shippingOriginNumber ?? ''} placeholder="1490" onChange={(event) => setContent({ ...content, shippingOriginNumber: event.target.value })} /></label>
              <label>Comuna<Input value={content.shippingOriginComuna ?? ''} placeholder="Cerro Navia" onChange={(event) => setContent({ ...content, shippingOriginComuna: event.target.value })} /></label>
              <label>Ciudad<Input value={content.shippingOriginCity ?? ''} placeholder="Santiago" onChange={(event) => setContent({ ...content, shippingOriginCity: event.target.value })} /></label>
              <label>Región<Input value={content.shippingOriginRegion ?? ''} placeholder="Región Metropolitana" onChange={(event) => setContent({ ...content, shippingOriginRegion: event.target.value })} /></label>
              <label>Teléfono de contacto para el retiro<Input type="tel" inputMode="tel" value={content.shippingOriginPhone ?? ''} placeholder={content.phone} onChange={(event) => setContent({ ...content, shippingOriginPhone: event.target.value })} /></label>
            </div>
          </details>
          <details className="admin-collapse"><summary>Avisos al celular</summary><p className="admin-section-note">Qué te avisa por notificación push en el celular (necesitas tenerlo activado arriba en "Avisarme ventas nuevas"). Todo viene activado por defecto.</p>
            <div className="form-grid">
              <label className="shipping-rate-address-toggle full"><input type="checkbox" checked={content.pushNewSale ?? true} onChange={(event) => setContent({ ...content, pushNewSale: event.target.checked })} /> Venta nueva confirmada</label>
              <label className="shipping-rate-address-toggle full"><input type="checkbox" checked={content.pushPendingTransfer ?? true} onChange={(event) => setContent({ ...content, pushPendingTransfer: event.target.checked })} /> Pedido pendiente de transferencia</label>
              <label className="shipping-rate-address-toggle full"><input type="checkbox" checked={content.pushPaymentReview ?? true} onChange={(event) => setContent({ ...content, pushPaymentReview: event.target.checked })} /> Pago recibido, revisar stock antes de despachar</label>
          ...
          <details className="admin-collapse"><summary>Pago por transferencia</summary><p className="admin-section-note">Si lo activas, el cliente puede elegir pagar por transferencia en vez de Mercado Pago (te ahorras la comisión). El pedido queda pendiente hasta que confirmes el pago a mano en Admin → Pedidos.</p>
            <div className="form-grid">
              ...
              <label className="full">Texto sobre la glosa/mensaje de la transferencia<Input value={content.transferGlosaText ?? ''} placeholder={'Pon {{orderId}} como mensaje/glosa de la transferencia, así identificamos tu pago al tiro.'} onChange={(event) => setContent({ ...content, transferGlosaText: event.target.value })} /><small className="password-hint">Usa {'{{orderId}}'} donde quieras que aparezca el número de pedido.</small></label>
              <label className="full">Texto de "envíame el comprobante"<Textarea rows={3} value={content.transferFollowupText ?? ''} placeholder={'Después de transferir, envíanos el comprobante por WhatsApp o respondiendo el correo de tu pedido. Reservamos tu pedido mientras confirmamos el pago.'} onChange={(event) => setContent({ ...content, transferFollowupText: event.target.value })} /><small className="password-hint">El enlace de WhatsApp se agrega automático al final, si tienes uno configurado en Contacto.</small></label>
            </div>
          </details>
